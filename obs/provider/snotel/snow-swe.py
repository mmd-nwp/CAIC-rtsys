...
# Loop through data and construct sql statement.
      ...
      for val in data.iter('values'):
        ...
        dateobj = datetime.datetime.strptime(time, '%Y-%m-%d %H:%M') + timedelta(hours=7)
...
<SOAP-ENV:Envelope xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/" xmlns:q0="http://www.wcc.nrcs.usda.gov/ns/awdbWebService" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  <SOAP-ENV:Body>
    <q0:getHourlyData>""" + triplets + """<elementCd>WTEQ</elementCd>
      <ordinal>1</ordinal><beginDate>""" + begdate + "</beginDate><endDate>" + enddate + """</endDate>
    </q0:getHourlyData>
  </SOAP-ENV:Body>
</SOAP-ENV:Envelope>"""
...
swexml = requests.post(url,data=body,headers=headers,verify=False)
#print(swexml.content)

print("SWE:")
root = ET.fromstring(swexml.content)
for data in root.iter():
    if data.tag=='return':
      x = data.find('beginDate')
      if x==None:
...
# Loop through data and construct sql statement.
      ...
      sql = "insert into obsSnow (time,staname,snowwater) values "
      check = 0
      for val in data.iter('values'):
        time = val.find('dateTime').text
...
# Insert data into db.
      ...
      if check > 0:
        sql = sql[:-1] + " on duplicate key update snowwater=values(snowwater)"
        print(sql)
        mycursor.execute(sql)
        mydb.commit()
