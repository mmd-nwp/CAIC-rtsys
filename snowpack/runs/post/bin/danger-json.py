...
def main():
    ...
    json_filename = f"/home/caic/www/snowpack/json/danger/{adate10[:6]}.json"

    # --- Set times.
    time_sec = adate_to_time(idate)
    date_str = time_to_adate(time_sec)
    date = adate_to_wrfdate(date_str)
    danger_time = date[0:10]

# Connect to the snowpack database.

    conn = create_connection("localhost", "caic", "steepndeep", "snowpack")

# Retrieve danger data.

    print(f"Read danger at: {danger_time}")
    query = "select polygon,atl,ntl,btl from polyDanger where date='"+danger_time+"'"
    results = execute_read_query(conn, query)

    nsta = len(results)
    print(f"No. polygon dangers: {nsta}")  
    zonedanger = {}
    if results:
        for row in results:
#           poly = f'{row[0]:03}'
            ...
            zonedanger[str(poly)] = feature

# Retrieve WRF grid points within polygons.

    query = "select id,zone,elev from zones_2km order by id"
    results = execute_read_query(conn, query)
    nsta = len(results)
    ...
    if results:
        for row in results:
            ...
            poly = str(row[1])
            elev = row[2]
            if poly in zonedanger:
                if elev > 3500:
                    danger = zonedanger[poly]["atl"]
                elif elev > 3200:
                    danger = zonedanger[poly]["ntl"]
                else:
                    danger = zonedanger[poly]["btl"]
                if (danger == ""):
                    danger = 0
            else:
                danger = 0
