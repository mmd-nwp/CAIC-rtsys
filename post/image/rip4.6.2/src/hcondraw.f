...
c                                                                     c
     ...
     &         lppcb,lsncb,lhscb,lptcb,llpcb,lppmm,lsncm,lprcb)
c
...
c
     ...
     &   lppcb(maxpl),lsncb(maxpl),lprcb(maxpl),
     &   lptcb(maxpl),llpcb(maxpl),lppmm(maxpl),lsncm(maxpl),
     &   lhscb(maxpl)
...
c
         ...
         elseif (llpcb(ipl)) then
            ...
            llbs( 6)='driz'
            llbs( 7)='frzd'
         elseif (cfeld(1,ipl)(1:3) .eq. 'ham' .or.
     &           cfeld(1,ipl)(1:3) .eq. 'hah') then
            nbox=5
            lfin(1)=2
            lfin(2)=3
            lfin(3)=4
