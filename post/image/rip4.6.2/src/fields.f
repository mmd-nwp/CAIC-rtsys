...
c
      elseif (cfeld(ifld,ipl)(1:5).eq.'ght1 ') then! geop. height, dm
         ...
         engplttl(ipl)='Geopotential height'
         unwk(ipl)='dm'
      elseif (cfeld(ifld,ipl)(1:5).eq.'intlw'.or.   ! integ. liquid water, mm
     +        cfeld(ifld,ipl)(1:6).eq.'intslw'.or.  ! integ. supercooled liquid water, mm
     +        cfeld(ifld,ipl)(1:5).eq.'intfz') then ! integ. frozen species, mm
         if (cfeld(ifld,ipl)(1:5).eq.'intlw') then
            iz = 0
            ic = 0
            engplttl(ipl)='Integ liquid water'
         elseif (cfeld(ifld,ipl)(1:6).eq.'intslw') then
            iz = 0
            ic = 1
            engplttl(ipl)='Integ supercooled liquid water'
         elseif (cfeld(ifld,ipl)(1:5).eq.'intfz') then
            iz = 1
            ic = 0
            engplttl(ipl)='Integ frozen hydrometeors'
         endif
         idimn(ipl)=2
         call getpt(miy,mjx,mkzh,ifree,2,i_pl2,wk,maxslab)
         call getpt(miy,mjx,mkzh,ifree,3,i_scr3a,wk,maxslab)
     ...
     &      miy,mjx,mkzh,3,1,scr3c,istat)
         ...
         if (iz.eq.0) then
            if (ic.eq.1) then
               ...
               do j=1,mjx-1
               do i=1,miy-1
                  if (tmk(i,j,k).lt.celkel)
     +               scr3b(i,j,k)=scr3c(i,j,k)*0.001
               enddo
               enddo
               enddo
            else
               call addorfill(scr3c,scr3b,miy,mjx,mkzh,3,1,.001,1.)
            endif
         endif
     ...
     &      miy,mjx,mkzh,3,1,scr3c,istat)
         ...
         if (iz.eq.0) then
            if (ic.eq.1) then
               ...
               do i=1,miy-1
                  if (tmk(i,j,k).lt.celkel)
     +               scr3b(i,j,k)=scr3b(i,j,k)+scr3c(i,j,k)*0.001
               enddo
               enddo
               enddo
            else
